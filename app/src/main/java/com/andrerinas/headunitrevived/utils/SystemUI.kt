    ...
    fun apply(window: Window, root: View, mode: Settings.FullscreenMode) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            if (controller != null) {
                when (mode) {
                    ...
                    Settings.FullscreenMode.LEGACY_FULLSCREEN -> {
                        // Pure legacy: only use FLAG_FULLSCREEN, skip WindowInsetsController entirely
            ...
            when (mode) {
                ...
                Settings.FullscreenMode.LEGACY_FULLSCREEN -> {
                    // Pure legacy: only use FLAG_FULLSCREEN, no immersive flags
                    window.addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN)
                    window.decorView.systemUiVisibility = (View.SYSTEM_UI_FLAG_FULLSCREEN
                            or View.SYSTEM_UI_FLAG_LOW_PROFILE)
                }
        ...
        ViewCompat.setOnApplyWindowInsetsListener(root) { v, insetsCompat ->
            if (mode == Settings.FullscreenMode.IMMERSIVE) {
                ...
                HeadUnitScreenConfig.updateInsets(manualL, manualT, manualR, manualB)
            } else if (mode == Settings.FullscreenMode.STATUS_ONLY || mode == Settings.FullscreenMode.LEGACY_FULLSCREEN) {
